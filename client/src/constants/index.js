// Calendar title used across the application
export const CALENDAR_TITLE = '📅 Agenda de la famille Le Guiberre';

// API configuration
export const API_URL = '/api/events';
export const REFRESH_INTERVAL = 5 * 60 * 1000; // 5 minutes
export const BUS_REFRESH_INTERVAL = 1 * 60 * 1000; // 1 minute for bus (more frequent updates needed)
export const HUE_REFRESH_INTERVAL = 3 * 1000; // 3 seconds for Hue lights (real-time updates)

// Screensaver configuration
export const SCREENSAVER_IDLE_TIME = 5 * 1000; // 5 seconds for testing (can be changed to minutes: 5 * 60 * 1000 for 5 minutes)

// Maximum number of events to display
export const MAX_EVENTS_WIDGET = 10; // Maximum events in the widget
export const MAX_DATES_WIDGET = 30; // Maximum date sections in the widget
export const MAX_EVENTS_PER_DATE_WIDGET = 2; // Maximum events per date in the widget
export const MAX_EVENTS_FULL_PAGE = null; // null = display all events (no limit)

